#include <JohnnyKalman.h>
 ...
 void JohnnyKalman::initial_kf_setup(LLAT_in gps_input){
	if (debugEnabled == 1) {
...
void JohnnyKalman::kalman_update(SpecGPS::LLA lla_ref, LLAT_in gps_input, Kalman_out filter_output) {
...
//Matrix
void JohnnyKalman::my_mx_vec_mult(double *mx_in, double *vec_in, double *vec_out, int n_dim) {
  int i, j;
  for (i = 0; i < n_dim; i++) {
    vec_out[i] = 0;
    for (j = 0; j < n_dim; j++) {
...
void JohnnyKalman::my_mx_add(double *mx_a, double *mx_b, double *mx_out, int n_dim) {
  int i, j;
  for (i = 0; i < n_dim; i++) {
...
void JohnnyKalman::my_mx_subtract(double *mx_a, double *mx_b, double *mx_out, int n_dim) {
...
void JohnnyKalman::my_mx_mx_mult(double *mx_a, double *mx_b, double *mx_out, int n_dim) {
  int i, j, k;
  double temp;
  int counter = 0;
  for (i = 0; i < n_dim; i++) {
...
void JohnnyKalman::my_mx_transpose(double *mx_in, double *mx_out, int n_dim) {
  int i, j;
  for (i = 0; i < n_dim; i++) {
    for (j = 0; j < n_dim; j++) {
...
void JohnnyKalman::my_scalar_mx_mult(double *mx_in, double *mx_out, double num_mult, int n_dim) {
  int i, j;
  for (i = 0; i < n_dim; i++) {
    for (j = 0; j < n_dim; j++) {
...
void JohnnyKalman::my_mx_copy(double *mx_in, double *mx_out, int n_dim) {
  int i, j;
  for (i = 0; i < n_dim; i++) {
    for (j = 0; j < n_dim; j++) {
...
void JohnnyKalman::my_mx_identity(double *mx_out, int n_dim) {
  int i, j;
  for (i = 0; i < n_dim; i++) {
    for (j = 0; j < n_dim; j++) {
      if (i == j) {
...
void JohnnyKalman::my_mx_2x2_inv(double *mx_in, double *mx_out) {
  int i, j;
...
//Vectors
void JohnnyKalman::my_vec_add(double *vec_a, double *vec_b, double *vec_out, int n_len) {
  //Add two vectors
  int i;
  for (i = 0; i < n_len; i++) {
...
void JohnnyKalman::my_vec_subtract(double *vec_first, double *vec_second, double *vec_out, int n_len) {
...
void JohnnyKalman::my_vec_scalar_mult(double *vec_in, double *vec_out, double num_mult, int n_len) {
  int i;
  for (i = 0; i < n_len; i++) {
...
void JohnnyKalman::my_vec_copy(double *vec_in, double *vec_out, int n_len) {
  int i;
  for (i = 0; i < n_len; i++) {
    *(vec_out + i) = *(vec_in + i);
  }
